...
const SCHEMA_SQL = [
  ...
  `CREATE TABLE IF NOT EXISTS pending (
    id TEXT PRIMARY KEY, text TEXT, assignedTo TEXT, completed TEXT,
...
async function handleGetPendingItems(params) {
  const user = await authenticate(params);
  if (!user) return { success: false, error: 'Authentication required' };
  // All authenticated users see every pending item (admin inputs + employee inputs)
  const rows = await exec('SELECT * FROM pending ORDER BY createdAt ASC', []);
  rows.sort((a, b) => (a.completed === 'true' ? 1 : 0) - (b.completed === 'true' ? 1 : 0));
  return { success: true, data: rows };
}
...
async function handleAddPendingItem(params) {
  ...
  await db.execute('INSERT INTO pending (id, text, assignedTo, completed, createdBy, createdAt) VALUES (?,?,?,?,?,?)',
    [String(Date.now()), text, String(params.assignedTo || ''), 'false', user.employeeId, new Date().toISOString()]);
  return { success: true };
}
...
async function handleUpdatePendingItem(params) {
  ...
  const completed = params.completed !== undefined ? String(params.completed) : null;
  const text = String(params.text || '');
  const rows = await exec('SELECT * FROM pending WHERE id = ?', [itemId]);
  if (!rows.length) return { success: false, error: 'Item not found' };
  // Any authenticated user can toggle completion / edit text (shared board)
  if (completed !== null) await db.execute('UPDATE pending SET completed = ? WHERE id = ?', [completed, itemId]);
...
async function seedIfEmpty() {
  ...
  for (const p of seedData.pending.data || []) {
    await db.execute('INSERT OR REPLACE INTO pending (id, text, assignedTo, completed, createdBy, createdAt) VALUES (?,?,?,?,?,?)',
      [String(p.id), String(p.text), String(p.assignedTo || ''), String(p.completed), String(p.createdBy), String(p.createdAt)]);
  }
